...
bool cc_walker::filter_read(const SeqLib::BamRecord &record) { return false; }

void cc_walker::load_intervals(uint32_t pad, string chr, uint32_t start,
                               uint32_t end) {
  ...
  for (auto &region : all_intervals) {
    ...
    if (chr_idx != -1 &&
        (region.chr != chr_idx || region.pos1 < start || region.pos2 > end)) {
      continue;
    }
    region.Pad(pad);
...
uint32_t cc_walker::n_overlap(const SeqLib::GenomicRegion &region,
                              uint32_t start, uint32_t end) {
  if (start > region.pos2 - this->pad || region.pos1 + this->pad > end)
    return 0;
  return MIN(region.pos2 - this->pad, end) -
         MAX(start, region.pos1 + this->pad);
}
...
bool cc_walker::walk_apply(const SeqLib::BamRecord &record) {
  ...
  if (region_idx != cur_region_idx) {
    // any reads left in the cache will all be in the previous region (which is
    // still called "cur_region")
    for (const auto &read : read_cache) {
      ...
      target_coverage.n_corrected +=
          n_overlap(cur_region, read.second.start, read.second.end);
  ...
  if (read_cache.find(read_name) == read_cache.end()) {
    ...
    target_coverage.n_corrected += n_overlap(
        cur_region, read_cache[read_name].start, record.PositionEnd());
    ...
    if (target_coverage.n_frags == 0) {
      ...
      target_coverage.var_fraglen +=
          fld * (fraglen - target_coverage.mean_fraglen);
    }
...
void cc_bin_walker::walk_all(string chr, uint32_t start, uint32_t end) {
  ...
  if (chr_idx == -1)
    ...
    curchr = chr_idx;
    walker::walk(
        SeqLib::GenomicRegion(chr_idx, start < 1000 ? 0 : (start - 1000), end));
    // NOTE: we add a 1kb buffer to the start to capture any readpairs upstream
    // of start that
    //       nonetheless might overlap it
  }
}

uint32_t cc_bin_walker::n_overlap(const uint32_t binstart, uint32_t binend,
                                  uint32_t start, uint32_t end) {
  if (start > binstart + this->binwidth || binstart > end)
    return 0;
...
bool cc_bin_walker::walk_apply(const SeqLib::BamRecord &record) {
  ...
  if (record_chr != curchr) {
    std::map<uint64_t, target_counts_t> ordered_active_bins(active_bins.begin(),
                                                            active_bins.end());
  ...
  for (auto bin = active_bins.begin(); bin != active_bins.end(); bin++) {
    if (record.Position() >= bin->first &&
        record.Position() < bin->first + binwidth) {
  ...
  if (read_cache.find(read_name) == read_cache.end()) {
    ...
    uint32_t midpoint =
        (read_cache[read_name].start + record.PositionEnd()) / 2;

    for (auto bin = active_bins.begin(); bin != active_bins.end(); bin++) {
      bin->second.n_corrected +=
          n_overlap(bin->first, bin->first + binwidth,
                    read_cache[read_name].start, record.PositionEnd());
...
bool cc_bin_walker_se::walk_apply(const SeqLib::BamRecord &record) {
  ...
  for (auto bin = active_bins.begin(); bin != active_bins.end(); bin++) {
    bin->second.n_corrected +=
        n_overlap(bin->first, bin->first + binwidth, record.Position(),
                  record.PositionEnd());
...
int main(int argc, char **argv) {
  ...
  if (!walker::basic_argparse(argc, argv, &args))
    exit(1);

  // parse args for trimming region
  CC::extra_args_t extra_args = {.chr = "-1", .start = 0, .end = 0xFFFFFFFF};
  ...
  if (use_bins) {
    if (!singleended) {
      CC::cc_bin_walker w =
          CC::cc_bin_walker(args.bam_in, stoi(args.input_file));
      if (!w.set_output_file(args.output_file))
        exit(1);
      w.walk_all(extra_args.chr, extra_args.start, extra_args.end);
    } else {
      CC::cc_bin_walker_se w =
          CC::cc_bin_walker_se(args.bam_in, stoi(args.input_file));
      if (!w.set_output_file(args.output_file))
        exit(1);
      w.walk_all(extra_args.chr, extra_args.start, extra_args.end);
    }
    ...
    throw runtime_error(
        "Invalid input to -i. Provide a positive integer for bin mode, or a "
        "valid file path to an interval file for interval mode.");
  }
